#!/usr/bin/env python3
"""Isolate whether the Aug 2 anomaly is a real site quirk or an artifact
of clicking multiple dates within one browser session: fresh page load,
click straight to Aug 2 only, nothing else."""
import datetime
import re
...
with sync_playwright() as pw:
    ...
    day = today + datetime.timedelta(days=TARGET_OFFSET_DAYS)
    print(f"Target date: {day.isoformat()}")
    found = find_and_click_date_tab(page, day)
    print(f"tab_found={found}")

    matches = page.get_by_text(re.compile("odyssey", re.IGNORECASE))
    count = matches.count()
    print(f"matching 'odyssey' elements: {count}")
    for i in range(count):
        ...
        for depth in [1, 2, 3, 4]:
            try:
                ...
                print(f"  depth {depth}: len={len(text)} text={text!r}")
